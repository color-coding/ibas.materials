...
public class testProduct extends TestCase {
    ...
    public void testMaterialPrice() throws Exception {
        ...
        if (opRstMaterialPrice.getError() != null) {
            throw opRstMaterialPrice.getError();
        }
        assertEquals(opRstMaterialPrice.getMessage(), opRstMaterialPrice.getResultCode(), 0);
        assertEquals("价格清单查询错误", opRstMaterialPrice.getResultObjects().firstOrDefault().getPrice().intValue(), 144);
    ...
    public void testMaterialQuantity() throws Exception {
        ...
        assertEquals(opRstMaterialQuantity.getMessage(), opRstMaterialQuantity.getResultCode(), 0);
        assertEquals("库存查询错误", opRstMaterialQuantity.getResultObjects().firstOrDefault().getOnHand().intValue(), 1100);
        //endregion
        //region 查询物料2的SH仓库的库存
        ICriteria criteria1 = new Criteria();
        ICondition condition1 = criteria1.getConditions().create();
        condition1.setAlias(Material.PROPERTY_CODE.getName());
        condition1.setValue(itemCode2);
