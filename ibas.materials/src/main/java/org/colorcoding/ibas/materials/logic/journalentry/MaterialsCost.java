...
public abstract class MaterialsCost extends JournalEntrySmartContent {
	...
	public final void caculate() throws Exception {
		...
		case INVENTORY:
			...
			if (this.isDiffMode() && preset != null) {
				// 价格差异 = 账面 - 实际历史成本
				this.setAmount(preset.subtract(this.getAmount() == null ? Decimals.VALUE_ZERO : this.getAmount()));
			}
			break;
		case NON_INVENTORY:
			if (this.isDiffMode()) {
	...
	protected void caculateNonInventoryCost(String itemCode) throws Exception {
		...
		this.setAmount(amount != null ? amount : Decimals.VALUE_ZERO);
	}
	...
	protected BigDecimal getLineBaseAmount() {
		if (this.getAmount() != null && !Decimals.isZero(this.getAmount())) {
