...
public class MaterialBatchJournal extends BusinessObject<MaterialBatchJournal>
	 ...
	 * 序列化版本标记
	 */
	private static final long serialVersionUID = -5705732090598055496L;

	/**
	 * 当前类型
	 */
	private static final Class<?> MY_CLASS = MaterialBatchJournal.class;

	/**
	 * 数据库表
	 */
	public static final String DB_TABLE_NAME = "${Company}_MM_MBT1";

	/**
	 * 业务对象编码
	 */
	public static final String BUSINESS_OBJECT_CODE = "${Company}_MM_BATCHJOURNAL";

	/**
	 * 业务对象名称
	 */
	public static final String BUSINESS_OBJECT_NAME = "MaterialBatchJournal";
	 ...
	 * 方向 属性
	 */
	@DbField(name = "Direction", type = DataType.ALPHANUMERIC, table = DB_TABLE_NAME, uniqueKey = true)
	 ...
	 * 计算价格 属性
	 */
	@DbField(name = "CalcPrice", type = DataType.DECIMAL, editType = EditType.PRICE, table = DB_TABLE_NAME)
	 ...
	 * 交易值 属性
	 */
	@DbField(name = "TransValue", type = DataType.DECIMAL, editType = EditType.SUM, table = DB_TABLE_NAME)
	 ...
	 * 到期日 属性
	 */
	@DbField(name = "DueDate", type = DataType.DATE, table = DB_TABLE_NAME)
	 ...
	 * 库存数量 属性
	 */
	@DbField(name = "StockQty", type = DataType.DECIMAL, editType = EditType.SUM, table = DB_TABLE_NAME)
	 ...
	 * 基于索引 属性
	 */
	@DbField(name = "BaseIndex", type = DataType.NUMERIC, table = DB_TABLE_NAME, uniqueKey = true)
	 ...
	 * 数据源 属性
	 */
	@DbField(name = "DataSource", type = DataType.ALPHANUMERIC, table = DB_TABLE_NAME, uniqueKey = true)
	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new IMaterialBatchInventoryContract() {
			...
			public BigDecimal getCalculatedPrice() {
				if (Strings.isNullOrEmpty(MaterialBatchJournal.this.getUpdateActionId())) {
					// 仅新状态影响成本
					if ((MaterialBatchJournal.this.getDirection() == emDirection.IN
							&& MaterialBatchJournal.this.getQuantity().compareTo(Decimals.VALUE_ZERO) > 0)
							|| (MaterialBatchJournal.this.getDirection() == emDirection.OUT
									&& MaterialBatchJournal.this.getQuantity().compareTo(Decimals.VALUE_ZERO) < 0)) {
						BigDecimal inventoryQuantity = Decimals.add(MaterialBatchJournal.this.getInventoryQuantity(),
								MaterialBatchJournal.this.getQuantity().abs());
						BigDecimal inventoryValue = Decimals.add(MaterialBatchJournal.this.getInventoryValue(),
