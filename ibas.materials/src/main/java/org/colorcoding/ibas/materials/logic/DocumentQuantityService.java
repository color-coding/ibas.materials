...
public abstract class DocumentQuantityService<L extends IBusinessLogicContract>
	...
	protected IDocumentCloseQuantityOperator fetchBeAffected(String documentType, Integer docEntry) {
		try {
			...
			IDocumentCloseQuantityOperator document = this.fetchBeAffected(criteria,
					IDocumentCloseQuantityOperator.class);
			if (document == null) {
				IDocumentFetcher<IDocumentCloseQuantityOperator> fetcher = DocumentFetcherManager.create()
	...
	static class _DocumentCloseQuantityOperator extends BusinessObject<IBODocument>
