...
public class MaterialSerialInventoryService
	...
	protected void impact(IMaterialSerialInventoryContract contract) {
		...
		if (materialSerial.isSavable()) {
			...
			if ((contract.getDirection() == emDirection.IN && contract.getQuantity().compareTo(Decimals.VALUE_ZERO) > 0)
	...
	private boolean isSafeUpdate() {
		if (this.getTrigger() instanceof IMaterialSerialJournal) {
			IMaterialSerialJournal triggerJournal = (IMaterialSerialJournal) this.getTrigger();
