...
public class MaterialIssueService extends BusinessLogic<IMaterialIssueContract, IMaterialInventoryJournal> {
	...
	private void checkContractData(IMaterialIssueContract contract) {
		if (contract.getJournal_IssueQuantity().compareTo(BigDecimal.ZERO) == 0) {
			throw new BusinessLogicException(I18N.prop("msg_if_issue_quantity_can't_be_zero"));
		}
		// region 查询物料
		ICriteria criteria = Criteria.create();
		ICondition condition = criteria.getConditions().create();
		...
		if (material.getInventoryItem() != emYesNo.NO) {
			throw new BusinessLogicException(String.format(I18N.prop("msg_if_material_is_not_inventory_item_can't_create_journal"),
		...
		if (material.getItemType() == emItemType.ITEM) {
			...
			if (warehouse == null) {
				throw new NullPointerException(String.format(I18N.prop("msg_if_warehouse_is_not_exist"),
