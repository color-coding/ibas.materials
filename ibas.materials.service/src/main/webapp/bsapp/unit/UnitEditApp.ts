 ...
 namespace materials {
    export namespace app {
        ...
        export class UnitEditApp extends ibas.BOEditApplication<IUnitEditView, bo.Unit> {
            ...
            /** 应用名称 */
            static APPLICATION_NAME: string = "materials_app_unit_edit";
            /** 业务对象编码 */
            ...
            protected viewShowed(): void {
                ...
                if (ibas.objects.isNull(this.editData)) {
                    // 创建编辑对象实例
                    this.editData = new bo.Unit();
                    this.proceeding(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_data_created_new"));
                }
                this.view.showUnit(this.editData);
            }
            run(): void;
            run(data: bo.Unit): void;
            run(): void {
                let that: this = this;
                if (ibas.objects.instanceOf(arguments[0], bo.Unit)) {
                    let data: bo.Unit = arguments[0];
                    // 新对象直接编辑
                    if (data.isNew) {
                        that.editData = data;
                        that.show();
                    ...
                    if (!ibas.objects.isNull(criteria) && criteria.conditions.length > 0) {
                        ...
                        boRepository.fetchUnit({
                            ...
                            onCompleted(opRslt: ibas.IOperationResult<bo.Unit>): void {
                                ...
                                if (ibas.objects.instanceOf(data, bo.Unit)) {
                                    // 查询到了有效数据
                                    that.editData = data;
                                    that.show();
                                } else {
        ...
        /** 视图-计量单位 */
        export interface IUnitEditView extends ibas.IBOEditView {
            /** 显示数据 */
