...
import { IMaterialGroupListView } from "../../../bsapp/materialgroup/index";
export class MaterialGroupListView extends ibas.BOListView implements IMaterialGroupListView {
    /** 返回查询的对象 */
    get queryTarget(): any {
            ...
            swipeContent: new sap.m.Toolbar("", {
                content: [
                    ...
                    new sap.m.Button("", {
                        ...
                        press(oEvent: any): void {
                            let parentControl: any = oEvent.getSource().getParent().getParent();
                            var editBo: bo.MaterialGroup = parentControl.getSwipedItem().getBindingContext().getObject();
    ...
    showData(datas: bo.MaterialGroup[]): void {
        ...
        if (!ibas.objects.isNull(model)) {
            // 已存在绑定数据，添加新的
            let hDatas: bo.MaterialGroup[] = (<any>model).getData();
