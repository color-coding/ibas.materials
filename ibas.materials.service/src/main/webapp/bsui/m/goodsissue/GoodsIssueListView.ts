...
import { GoodsIssueListView as cGoodsIssueListView } from "../../c/goodsissue/index";
...
export class GoodsIssueListView extends ibas.BOListView implements IGoodsIssueListView {
            ...
            swipeContent: new sap.m.Toolbar("", {
                content: [
                    ...
                    new sap.m.Button("", {
                        ...
                        press(oEvent: any): void {
                            let parentControl: any = oEvent.getSource().getParent().getParent();
                            var editBo: bo.GoodsIssue = parentControl.getSwipedItem().getBindingContext().getObject();
    ...
    showData(datas: bo.GoodsIssue[]): void {
        ...
        if (!ibas.objects.isNull(model)) {
            // 已存在绑定数据，添加新的
            let hDatas: bo.GoodsIssue[] = (<any>model).getData();
